...
class HuaweiSpider(scrapy.Spider):
    ...
    def parse_item(self, response):
        page = Selector(response)
        item = AppstoreItem()
        item['title'] = page.xpath('//ul[@class="app-info-ul nofloat"]/li/p/' \
                'span[@class="title"]/text()').extract_first().encode('utf-8')
        item['app_icon'] = page.xpath('//ul[@class="app-info-ul nofloat"]/li/' \
                'img/@lazyload').extract_first()
        ...
        for div in divs:
            ...
            recomm_appid = re.match(r'http://.*/(.*)', url).group(1)
            name = div.xpath('./p[@class="name"]/a/text()').extract_first().encode('utf-8')
            recomm += "{0}:{1},".format(recomm_appid, name)
        print recomm
        item['recomm'] = recomm
        yield item
